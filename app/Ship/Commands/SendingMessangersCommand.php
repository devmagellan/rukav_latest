...
class SendingMessangersCommand extends ConsoleCommand
{
	...
	//\Log::info('Days',array($date));
     $connects=\App\Containers\Connect\Models\Connect::where('created_at', '>=', $date)->where('viewed_at', null)->where('mail_viewed_at', null)->where('text','!=', null)->with('receiver')->get();
     ...
     $connect->mail_viewed_at=$mutable;
     $connect->save();
	 }}
