...
use App\Containers\User\Notifications\CompanySocialRegistrationDone;
use Log;

class SendSocialEmailVerification implements ShouldQueue
{
...
\Log::info('emailVerificationCodeinObject'.$this->user->emailCode);
	...
	\Log::info('emailVerificationCodeinSession2'.session()->get('emailVerificationCode'));
		...
		$this->user->notify(new CompanySocialRegistrationDone($this->user));
