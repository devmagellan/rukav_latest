...
<? $type_name=[];?>
        ...
        <?
        $permissions_role=\App\Containers\Authorization\Models\RoleHasPermission::where('role_id',$role_id)->where('permission_id',$perm->id)->with('permission_item')->first();
        ?>
                                ...
                                <h3>{{$type_name_show}}</h3>
                                    ...
                                    @if($permissions_role && in_array($perm->id,$available_permissions_final_array))
                                            <label class="kt-checkbox">
                                                ...
                                                <input type="checkbox" checked="checked"> {{$permissions_role->permission_item->description}}
                                                </div>
                                                    <span></span>
                                            </label>
                            </br>
                                ...
                                @else
                <?
                $permissions=\App\Containers\Authorization\Models\Permission::where('id',$perm->id)->first();
                ?>
                    <label class="kt-checkbox">
                        <div class="permission_process">
                            <input type="hidden" class="process_id" value="{{$permissions->id}}">
                        <input type="checkbox" > {{$permissions->description}}
                        </div>
                        <span></span>
                    </label></br>
                                @endif
