...
if(null!=$ad->getSender ){
  ...
  $www='Не определено';
  $address='Не определено';}
  if($ad->language_filter!=null){
	 // \Log::info('JSON=>',json_decode($ad->language_filter)); 
	   \Log::info('JSON=>'.implode(json_decode($ad->language_filter))); 
  }
	  ...
	  $nestedData [21] = ($ad->language_filter!=null) ? implode(json_decode($ad->language_filter))  : 'Не определено';
      $data [] = $nestedData;
    }
    /*
